...
class ValyrianTranslate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      translatedText: "",
  ...
  handleSubmit(event) {
    ...
    const headers = {
      "X-FunTranslations-Api-Secret":
        process.env.REACT_APP_HEADER_VALUE_VALYRIAN
    };
    axios
      .get(
        `https://api.funtranslations.com/translate/valyrian.json?text=${this.state.texttoTranslate
          .trim()
          .replace(/\s/g, "%20")}`,
        { headers }
      )
  ...
  render() {
    return (
      <div className="valyrian-background">
        <div className="translate-container" style={{ marginTop: this.props.dropdown ? '160px' : '0px' }} >
          <form onSubmit={this.handleSubmit}>
            <textarea
              ...
              placeholder="Enter text to translate to Valyrian"
            />
            <input
...
export default ValyrianTranslate;
