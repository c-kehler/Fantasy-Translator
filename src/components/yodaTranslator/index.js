...
class YodaTranslate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      translatedText: "",
  ...
  handleSubmit(event) {
    ...
    const headers = {
      "X-FunTranslations-Api-Secret": process.env.REACT_APP_HEADER_VALUE_YODA
    };
    axios
      .get(
        `https://api.funtranslations.com/translate/yoda.json?text=${this.state.texttoTranslate
          .trim()
          .replace(/\s/g, "%20")}`,
        { headers }
      )
          ...
          <div className="translate-container" style={{ marginTop: this.props.dropdown ? '160px' : '0px' }} >
            <form onSubmit={this.handleSubmit}>
              <textarea
                ...
                placeholder="Enter text to translate to Yoda speak"
              />
              <button
                ...
                type="submit"
                value="Translate"
              >Translate <i class="fab fa-jedi-order fa-lg" aria-hidden="true" /></button>
